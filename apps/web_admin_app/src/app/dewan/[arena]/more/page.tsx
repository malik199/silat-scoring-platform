                  ...
                  <tbody>
                    {judgeOrder.map((judgeId, i) => {
                      ...
                      const seatNum = (judgeSeats && Object.keys(judgeSeats).length > 0)
                        ...
                        : String(i + 1);
                      const displayName = t?.name || judgeSeats?.[seatNum ?? ""]?.name || t?.email || `Judge ${seatNum ?? i + 1}`;
                      const displayEmail = t?.email || judgeSeats?.[seatNum ?? ""]?.email || "";
                      return (
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center gap-2.5">
                              ...
                              <div className="flex flex-col min-w-0">
                                <span className="text-sm font-medium text-primary truncate">{displayName}</span>
                                {displayName !== displayEmail && displayEmail && <span className="text-xs text-muted truncate">{displayEmail}</span>}
                              </div>
                            </div>
                          </td>
