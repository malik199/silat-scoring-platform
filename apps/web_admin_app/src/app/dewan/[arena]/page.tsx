...
export default function DewanPage() {
  ...
  return (
    <Shell title={`Dewan — Arena ${arenaNumber}`}>
        ...
        {match === undefined ? (
          ...
          <div className="flex items-center gap-4">
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold uppercase tracking-widest text-danger mb-0.5">Red Corner</p>
              <p className="text-base font-bold text-primary truncate">
                {redComp ? `${redComp.firstName} ${redComp.lastName}` : "—"}
              </p>
              {redComp && <p className="text-xs text-muted truncate">{redComp.schoolName || redComp.country || "—"}</p>}
            </div>
            <div className="text-xs font-bold text-muted flex-shrink-0">VS</div>
            <div className="flex-1 min-w-0 text-right">
              <p className="text-xs font-semibold uppercase tracking-widest text-blue-400 mb-0.5">Blue Corner</p>
              <p className="text-base font-bold text-primary truncate">
                {blueComp ? `${blueComp.firstName} ${blueComp.lastName}` : "—"}
              </p>
              {blueComp && <p className="text-xs text-muted truncate">{blueComp.schoolName || blueComp.country || "—"}</p>}
            </div>
            <div className="ml-4 flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-warn/10 border border-warn/30">
      ...
      {match && (<>
        ...
        <div className="bg-surface border border-border rounded-xl overflow-hidden mb-4">
          <div className="grid grid-cols-3">
            ...
            <div className="flex flex-col items-center justify-center py-6 border-x border-border gap-1">
              <p className="text-sm font-semibold text-secondary">Total Score</p>
              <p className="text-xs text-muted">≥ 2 judges · 5 s window</p>
              {scoreEvents.length > 0 && (
                <p className="text-xs text-muted">{confirmedTaps}/{scoreEvents.length} taps confirmed</p>
              )}
              {judgeOrder.length > 0 && (
                <p className="text-xs text-muted">{judgeOrder.length} judge{judgeOrder.length !== 1 ? "s" : ""}</p>
              )}
            </div>
